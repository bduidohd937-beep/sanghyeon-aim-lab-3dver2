  ...
  function TrainingSetup({
    ...
    const info = {
      flick: {
        ...
        korean: '순간 조준',
        desc: '출현하는 타겟을 빠르게 포착하고 정확하게 클릭합니다.',
        options: ['타겟 랜덤 출현', '헤드라인 기준', '빠른 첫 조준'],
      },
      tracking: {
        icon: '👁️',
        title: 'TRACKING',
        korean: '추적 조준',
        desc: '움직이는 타겟을 따라가며 안정적인 조준을 훈련합니다.',
        options: ['발로란트식 스트레이프', '이동 방향 랜덤', '속도 변화'],
      },
      braking: {
        icon: '🛑',
        title: 'BRAKING',
        korean: '감속 조준',
        desc: '이동을 멈추는 순간 정확하게 첫 발을 맞춥니다.',
        options: ['이동 거리 랜덤', '정지 타이밍 랜덤', '멈춘 뒤 클릭'],
      },
    }[drill];

    const difficulties = [
      { id: 'trainee', title: '응애 나 뉴비에요', desc: '에임 기본기부터 천천히', level: 'TRAINEE' },
      { id: 'rookie', title: '이제 사람 구실 좀 해볼게요', desc: '실전 감각을 익히는 단계', level: 'ROOKIE' },
      { id: 'operator', title: '나 정도면 실력자지', desc: '경쟁전 기준의 훈련 강도', level: 'OPERATOR' },
      { id: 'elite', title: '경쟁에서 캐리할게요', desc: '최고 난도의 집중 훈련', level: 'ELITE', hell: true },
    ];

    return (
      <main className="setup-screen">
        <button className="setup-back" onClick={onBack}>← 훈련 선택으로</button>

        <header className="setup-header">
          <span className="setup-eyebrow">TRAINING PROTOCOL // 3D RANGE</span>
          <div className="setup-mode">
            <div className="setup-mode-icon">{info.icon}</div>
            <div>
              <h1>{info.title} · {info.korean}</h1>
              <p>{info.desc}</p>
            </div>
          </div>
        </header>
        ...
        <section className="setup-section">
          ...
          <div className="difficulty-grid">
            {difficulties.map((item) => (
              <button
                key={item.id}
                className={`difficulty-card ${item.hell ? 'hell' : ''} ${difficulty === item.id ? 'selected' : ''}`}
                onClick={() => setDifficulty(item.id)}
              >
                <strong>{item.title}</strong>
                <span>{item.desc}</span>
                <b>{difficulty === item.id ? 'SELECTED' : item.level}</b>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="setup-section">
          ...
          <div className="duration-grid">
            {[15, 30, 60].map((value) => (
              <button
                key={value}
                className={`duration-card ${duration === value ? 'selected' : ''}`}
                onClick={() => setDuration(value)}
              >
                <strong>{value}s</strong>
                <span>{value === 15 ? '빠른 손풀기' : value === 30 ? '표준 훈련' : '집중 훈련'}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="setup-section">
          <div className="setup-section-head">
            <span>03</span>
            <div>
              <h2>{info.title} 훈련 옵션</h2>
              <p>3D 훈련장에서 적용될 핵심 훈련 요소입니다.</p>
            </div>
          </div>
        ...
        <footer className="setup-footer">
          <div>
            <span>READY PROTOCOL</span>
            <strong>{info.title} · {difficulty.toUpperCase()} · {duration} SEC</strong>
          </div>
          <button
            className="setup-start"
